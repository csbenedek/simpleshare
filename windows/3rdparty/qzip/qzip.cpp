...
public:
    ...
    void fillFileInfo(int index, QZip::QZipReader::FileInfo &fileInfo) const;
...
void QZipPrivate::fillFileInfo(int index, QZip::QZipReader::FileInfo &fileInfo) const
{
...
public:
    QZipReaderPrivate(QIODevice *device, bool ownDev)
        : QZipPrivate(device, ownDev), status(QZip::QZipReader::NoError)
    {
    }

    void scanFiles();

    QZip::QZipReader::Status status;
};
...
public:
    QZipWriterPrivate(QIODevice *device, bool ownDev)
        ...
        status(QZip::QZipWriter::NoError),
        permissions(QFile::ReadOwner | QFile::WriteOwner),
        compressionPolicy(QZip::QZipWriter::AlwaysCompress)
    {
    }

    QZip::QZipWriter::Status status;
    QFile::Permissions permissions;
    QZip::QZipWriter::CompressionPolicy compressionPolicy;
...
void QZipWriterPrivate::addEntry(EntryType type, const QString &fileName, const QByteArray &contents/*, QFile::Permissions permissions, QZip::Method m*/)
{
    ...
    if (! (device->isOpen() || device->open(QIODevice::WriteOnly))) {
        status = QZip::QZipWriter::FileOpenError;
        return;
    }
    device->seek(start_of_directory);

    // don't compress small files
    QZip::QZipWriter::CompressionPolicy compression = compressionPolicy;
    if (compressionPolicy == QZip::QZipWriter::AutoCompress) {
        if (contents.length() < 64)
            compression = QZip::QZipWriter::NeverCompress;
        else
            compression = QZip::QZipWriter::AlwaysCompress;
    }
    ...
    if (compression == QZip::QZipWriter::AlwaysCompress) {
...
QString QZipReader::entryPathAt(int index) const
{
    return entryInfoAt(index).filePath;
}
