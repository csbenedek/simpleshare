...
private slots:
    ...
    void signUpClicked(const QUrl& link);
    ...
    void onUploadFailed(const QString& fileName, BxNet::RESPONSE_STATUS status);
    void onAuthError(BxNet::RESPONSE_STATUS err);
    void onNotLoggedIn();
    void onChangeUserIcon(QPixmap);
    void onLogin();
    void onLogout();
    void onCreateFolderError(BxNet::RESPONSE_STATUS);
    ...
    void updateFoldersList(const QString folderName, const QString folderId);
    void folderTextChanged(QString folderName);
    void folderTextFinished();

    void messageClicked();

    void on_viewFolder_pushButton_clicked();
    ...
    void on_userEmailLineEdit_returnPressed();
...
private:
    ...
    QMap<QString, QString>  m_folders;
    bool                    m_blockFolderModifications;
};
