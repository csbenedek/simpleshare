...
+ (BoxUser*)userWithSavedAttributes:(NSDictionary *)savedAttributes {
    BoxUser* user = [[BoxUser alloc] init];
    [user loadFromDictionary:savedAttributes];
    return [user autorelease];
}
...
+ (void)removeSavedUser {
    ...
    if ([fileManager fileExistsAtPath:fileLocation]) {
        [fileManager removeItemAtURL:[NSURL fileURLWithPath:fileLocation] error:NULL];
    }                             
}
