...
- (void)setValuesWithDictionary:(NSDictionary *)values {
	...
	if([[values objectForKey:@"shared_link"] length]) {
...
- (NSMutableDictionary *)getValuesInDictionaryForm {
	...
	if(self.objectCreatedTime) {
		[returnDict setObject:[NSString stringWithFormat:@"%.0f", [self.objectCreatedTime timeIntervalSince1970]] forKey:@"created"];
	}
	if(self.objectUpdatedTime) {
		[returnDict setObject:[NSString stringWithFormat:@"%.0f", [self.objectUpdatedTime timeIntervalSince1970]] forKey:@"updated"];
	}
	if(self.objectPath) {
