...
-(oneway void) release
{
    AT_SYNCHRONIZED(self)
    {
        //DbgLog(@"%@ (%d)", self, [self retainCount]);
...
-(void) dealloc
{
	...
	while( (fdNum = [enny nextObject]) )
	{
    	if( close( [fdNum intValue] ) == -1 )
            DbgLog(@"dealloc: Couldn't close file descriptor (%d)", errno);
    }
...
-(void) removePathFromQueue: (NSString*)path
{
	...
	if( close( fd ) == -1 )
        DbgLog(@"removePathFromQueue: Couldn't close file descriptor (%d)", errno);
}
...
-(void)		watcherThread: (id)sender
{
    ...
    while( keepThreadRunning )
    {
			...
			if( n > 0 )
			{
				if( ev.filter == EVFILT_VNODE )
				{
					if( ev.fflags )
					{
						...
						//DbgLog(@"UKKQueue: Detected file change: %@", fpath);
						[[NSWorkspace sharedWorkspace] noteFileSystemChanged: fpath];
						
						//DbgLog(@"ev.flags = %u",ev.fflags);	// DEBUG ONLY!
		...
		NS_HANDLER
			DbgLog(@"Error in UKKQueue watcherThread: %@",localException);
		NS_ENDHANDLER
	...
	if( close( theFD ) == -1 )
		DbgLog(@"release: Couldn't close main kqueue (%d)", errno);
	
    //DbgLog(@"exiting kqueue watcher thread.");
}
...
//      used by DbgLog & co. when you request to print this object using the
//      %@ format specifier.
//
//	REVISIONS:
//		2004-11-12	UK	Created.
